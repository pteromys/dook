use dook::{ConfigLoader, DownloadsPolicy, Loader, QueryCompiler};
...
pub fn get_query_compiler() -> QueryCompiler {
    ...
    QueryCompiler::new(ConfigLoader::new(None), language_loader)
}

fn get_downloads_policy() -> DownloadsPolicy {
    let settings_path = option_env!("CARGO_TARGET_TMPDIR")
        .map(|d| std::path::PathBuf::from(d).join("downloads_policy.txt"));
    dook::get_downloads_policy_from_path(settings_path.as_ref())
}
